...
export async function loginUser(email, password) {
  const response = await fetch(`${API_URL}/login`, { //sends a post request (for sending data)
    method: "POST",
    headers: {
  ...
  if (response.status === 200 || response.status === 201) {  //200 = ok/success, 201 = created
...
export async function registerUser(name, email, password) {
  const response = await fetch(`${API_URL}/users`, { //sends a post request 
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
...
export async function deleteUserByName(name) {
  const response = await fetch(`${API_URL}/users/${name}`, { //delete request
    method: "DELETE",
  });
